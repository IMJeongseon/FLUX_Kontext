"""FLUX.1-Kontext + 3-region regional binding:
  1) dog area           → 'cat'
  2) original house area → remove (suppress house ref + all object text) → grass fill
  3) distant 1-o'clock  → 'small distant blurry cat tower'
Post-process DoF blur on the distant cat-tower region.
"""
...
def main():
    ...
    parser.add_argument("--house-add-mask", default=str(HERE / "data" / "mask_house_distant.png"))
    parser.add_argument("--output", default=str(HERE / "outputs" / "cat_edit_3region.png"))
    parser.add_argument(
        "--prompt",
    ...
    parser.add_argument("--blur-feather", type=int, default=14)
    ...
    rem_mask = Image.open(args.house_remove_mask).convert("L")
    add_mask = Image.open(args.house_add_mask).convert("L")

    dog_idx = mask_to_token_indices(dog_mask, grid_size=GRID)
    rem_idx = mask_to_token_indices(rem_mask, grid_size=GRID)
    add_idx = mask_to_token_indices(add_mask, grid_size=GRID)
    print(f"dog: {len(dog_idx)}, house-remove: {len(rem_idx)}, house-add: {len(add_idx)} tokens")

    # Region label per gen token: 0=dog, 1=remove, 2=add
    # 우선순위: dog > add > remove (add가 remove 안에 있어도 add가 이김)
    seen = {}
    all_gen = []
    for g in dog_idx:
        if g not in seen:
    ...
    # 10:with 11:_ 12:a 13:small 14:distant 15:blur 16:ry 17:cat 18:tower 19:.
    dog_src = [2]
    cat_span = [6]
    house_src = [9]
    tower_span = [13, 14, 15, 16, 17, 18]  # "small distant blurry cat tower"
    all_object_text = dog_src + cat_span + house_src + tower_span
    ...
    for i, g in enumerate(all_gen):
        lbl = seen[g]
        if lbl == 0:  # dog area → cat
            for t in house_src + tower_span:
                text_bias[i, t] = args.suppression_bias
            for r in dog_ref_set:
                ref_bias[i, r] = args.suppression_bias
        elif lbl == 1:  # remove area → grass fill
            for t in all_object_text:
                text_bias[i, t] = args.suppression_bias
            for r in rem_ref_set:
                ref_bias[i, r] = args.suppression_bias
        else:  # lbl == 2, add area → small distant blurry cat tower
            for t in dog_src + cat_span:
                text_bias[i, t] = args.suppression_bias
            # No image suppression needed (bg content at that location is trees/grass)

    print(f"text_bias nonzeros: {(text_bias != 0).sum().item()}, ref_bias nonzeros: {(ref_bias != 0).sum().item()}")
    ...
    # DoF blur on the distant cat-tower region
