"""FLUX.1-Kontext + MRAS attention suppression + RePaint background anchoring.

mask 영역의 gen 토큰이 원본 이미지의 dog/house를 참조하지 않도록 attention
suppression을 걸고, 배경은 RePaint로 유지한다.
"""
...
def make_repaint_callback(proc, src_packed, bg_tensor):
    ...
    def callback(pipeline, step_idx, timestep, kwargs):
        proc.current_step = step_idx  # MRAS step update
...
def main():
    ...
    parser.add_argument("--output", default=str(HERE / "outputs" / "cat_edit_mras.png"))
    parser.add_argument(
        "--prompt",
    ...
    proc, _orig = install_mras_processors(
        pipe,
        gen_mask_indices=mask_idx,
        ref_mask_indices=mask_idx,
        suppression_bias=args.suppression_bias,
