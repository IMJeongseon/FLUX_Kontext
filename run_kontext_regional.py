"""FLUX.1-Kontext + regional prompt binding + per-object image suppression + RePaint.

- dog 영역 gen 토큰: text[cat_tower_span, house_span] 억제, ref[dog_region] 억제
- house 영역 gen 토큰: text[cat_span, dog_span] 억제, ref[house_region] 억제
- 배경: RePaint 로 원본 latent 앵커링
"""
...
def main():
    ...
    parser.add_argument("--output", default=str(HERE / "outputs" / "cat_edit_regional.png"))
    parser.add_argument(
        "--prompt",
    ...
    # Union (with tag which object each belongs to)
    seen = set()
    all_gen = []
    is_dog = []  # parallel list: True if the i-th gen token is dog, else house
    for g in dog_idx:
        if g not in seen:
            all_gen.append(g)
            is_dog.append(True)
    ...
    # Text spans (from T5 tokenization of prompt)
    # 0:Replace 1:the 2:dog 3:with 4:_ 5:a 6:cat 7:and 8:the 9:house 10:with 11:_ 12:a 13:cat 14:tower
    dog_source_span = [2]         # "dog"
    cat_span = [6]                # "cat" (dog destination)
    house_source_span = [9]       # "house"
    cat_tower_span = [13, 14]     # "cat tower" (house destination)
    ...
    for i, (g, is_d) in enumerate(zip(all_gen, is_dog)):
        if is_d:
            # dog region: suppress text 'house', 'cat tower', and its own dog ref
            ...
            # house region: suppress text 'dog', 'cat', and its own house ref
