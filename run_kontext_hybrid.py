"""FLUX.1-Kontext + MRAS union ref suppression + Regional text suppression + RePaint.

- ref suppression: 모든 union gen 토큰이 모든 union ref 토큰 참조 차단 (MRAS 스타일)
- text suppression: dog 영역 gen → cat tower 텍스트 억제, house 영역 gen → cat 텍스트 억제
- 결과: house 영역이 새 tower 를 강하게 생성하면서, 그 영역에 여분 고양이가 얹히지 않음
"""
...
def main():
    ...
    parser.add_argument("--output", default=str(HERE / "outputs" / "cat_edit_hybrid.png"))
    parser.add_argument(
        "--prompt",
        default="Replace the dog with a cat and the house with a cat tower. Keep each replacement object in the exact same position, scale, and orientation as the original object.",
    )
    ...
    house_idx = mask_to_token_indices(Image.open(args.house_mask).convert("L"), grid_size=GRID)
    print(f"dog: {len(dog_idx)}, house: {len(house_idx)} tokens")
    ...
    # Prompt token positions (position-preserving prompt):
    # 0:Replace 1:the 2:dog 3:with 4:_ 5:a 6:cat 7:and 8:the 9:house
    # 10:with 11:_ 12:a 13:cat 14:tower 15:.
    dog_src = [2]
    cat_span = [6]
    house_src = [9]
    cat_tower_span = [13, 14]

    text_bias = torch.zeros(n_gen, T_FULL, dtype=torch.float32)
    ref_bias = torch.zeros(n_gen, N, dtype=torch.float32)

    # MRAS union ref suppression: 모든 gen 토큰이 union ref 참조 차단
    union_ref = list(seen)
    for i in range(n_gen):
        ref_bias[i, union_ref] = args.suppression_bias

    # Regional text suppression
    for i, is_d in enumerate(is_dog):
        if is_d:
            for t in house_src + cat_tower_span:
